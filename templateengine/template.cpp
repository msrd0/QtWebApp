...
int Template::setLoop(const QString &name, const QStringList &data)
{
	int count = loop(name, data.length());
	for (int i = 0; i < data.length(); i++)
	{
		QString id = QString::number(i);
		setVariable(name + id + ".id", id);
		setVariable(name + id + ".val", data[i]);
	}
	return count;
}

int Template::setLoop(const QString &name, const QList<QVariant> &data)
{
	int count = loop(name, data.length());
	for (int i = 0; i < data.length(); i++)
	{
		QString id = QString::number(i);
		setVariable(name + id + ".id", id);
		setVariable(name + id + ".val", data[i].toString());
	}
	return count;
}
